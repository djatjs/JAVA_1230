...
- TRIGGER_EVENT : 트리거가 실행되는 이벤트. INSERT | UPDATEM | DELETE
- 트리거 안에서 사용되는 키워드
	- OLD : DELETE, UPDATE 하기 전의 데이터. OLD.속성을 통해 활용 가능
    - NEW : INSERT, UPDATE 한 후의 데이터. NEW.속성을 통해 활용 가능
*/
...
BEGIN
    ...
    UPDATE AVERAGE
	SET 
		AV_SUM = AV_SUM + NEW.SC_SCORE, # SCORE에 INSERT한 후에는 NEW에 담겨져 있음
		AV_COUNT = AV_COUNT +1
	WHERE
