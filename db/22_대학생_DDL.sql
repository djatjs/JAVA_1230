...
CREATE TABLE `FINAL_PASS` (
	...
	`FP_RES_NUM`	CHAR(14)UNIQUE  NOT	NULL,
...
DROP TABLE IF EXISTS `SUBJCET_CATEGORY`;

CREATE TABLE `SUBJCET_CATEGORY` (
...
DROP TABLE IF EXISTS `LETURE_STANDARD`;

CREATE TABLE `LETURE_STANDARD` (
...
ALTER TABLE `SUBJECT` ADD CONSTRAINT `FK_SUBJCET_CATEGORY_TO_SUBJECT_1` FOREIGN KEY (
	`SJ_SC_CODE`
)
REFERENCES `SUBJCET_CATEGORY` (
	`SC_CODE`
);
...
ALTER TABLE `LETURE_STANDARD` ADD CONSTRAINT `FK_LECTURE_TO_LETURE_STANDARD_1` FOREIGN KEY (
	`LS_LE_NUM`
)
REFERENCES `LECTURE` (
	`LE_NUM`
