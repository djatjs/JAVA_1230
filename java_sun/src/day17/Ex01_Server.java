...
public class Ex01_Server {
	...
	private static void ranking() {
		// list 보내줌 
		// 삭제 안하고 정렬된 리스트 중 항목 5개만 가져와서 보여줌
		List<Game> subList;
		try {
		    if (list.size() > 5) {
		        // 리스트 크기가 5 이상일 때는 첫 5개 요소만 추출
		        subList = new ArrayList<>(list.subList(0, 5));
		    } else {
		        // 리스트 크기가 5 미만일 때는 전체 리스트 복사
		        subList = new ArrayList<>(list.subList(0, list.size()));
		    }
		    
		    //subList = list.subList(0, Math.min(0,list.size()));
		    oos.writeObject(subList);
		    oos.flush();
		    // oos.reset();  // 필요 없다면 이 라인 제거
		} catch (IOException e) {
		    e.printStackTrace();
		}
	}
