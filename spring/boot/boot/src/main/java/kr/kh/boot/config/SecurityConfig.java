    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http.csrf(csrf ->csrf.disable())
            .authorizeHttpRequests((requests) -> requests
                ...
                .requestMatchers("/post/insert/*","/post/delete/*")
                .hasAnyAuthority(UserRole.USER.name(), UserRole.ADMIN.name())
                //관리자만 접근 가능한 URL(AdminInterceptor 역할)
            ...
            .formLogin((form) -> form
                ...
                .loginProcessingUrl("/login")//로그인 화면에서 로그인을 눌렀을 때 처리할 url 지정
                .defaultSuccessUrl("/") //로그인 성공 후 보낼 URL
            )
            //자동 로그인
            .rememberMe(rm-> rm
                .userDetailsService(memberDetailService) //자동 로그인 할 때 사용할 userDetailService를 추가
                .key(rememberMeKey) //키가 변경되면 기존 토큰이 무효처리
                .rememberMeCookieName("LC")  //쿠키 이름
                .tokenValiditySeconds(60*60*24*7) //유지 기간 : 7일
            )
            //로그아웃. 등록된 url에 post방식으로 요청이 온 경우 자동으로 로그아웃 처리.
            .logout((logout) -> logout
            		.logoutUrl("/logout") //로그아웃을 처리할 URL 지정. post 형식
